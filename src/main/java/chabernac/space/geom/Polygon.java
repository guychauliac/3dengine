...
public class Polygon implements iTranslatable {
  ...
  private void initialize() {
    ...
    // myColor = new Color((int)Math.random() % 255, (int)Math.random() % 255,
    // (int)Math.random() % 255);
    // Debug.log(Polygon.class,"color: " + System.currentTimeMillis() % 255);
    /*
     * int red = Math.abs((int)System.currentTimeMillis()) % 255; int green =
     * Math.abs((int)System.currentTimeMillis()) % 255; int blue =
     * Math.abs((int)System.currentTimeMillis()) % 255;
     */
    int red = (int) (Math.random() * 255);
    int green = (int) (Math.random() * 255);
    int blue = (int) (Math.random() * 255);
    // Debug.log(Polygon.class,"red: " + red + " green: " + green + " blue: " +
    // blue);
    if (color == null)
  ...
  public void createTexture() {
    try {
      if (myTexture == null) {
        // we take the x vector of the texture parallel to the first 2 points of the
        // polygon
        Point3D theOrigin = w[0].myPoint;
        GVector theXVector = new GVector(w[0].myPoint, w[1].myPoint);
        // now we multiply the x vector with the plane's normal vector to obtain the y
        // vector in the plane and orthogonal with the x vector
        ...
        if (myTextureName != null) {
          myTextureImage = TextureFactory.getTexture(myTextureName, isTransparentTexture);
          theXVector.multiply(myTextureImage.width);
          theYVector.multiply(myTextureImage.height);
        }
        ...
        if (myTextureImage != null) {
          myTexture = new Texture2(theOrigin, theXVector, theYVector, myTextureImage);
        } else {
          // there is not texture image, but still we create a texture and say that is
          // always have to return the color of the polygon
          // this way we can still use the texture object for calculating camera and real
          // world points for points on the screen
  ...
  /*
   * a polygon which is convex can only contain 1 vertex more after clipping to a
   * frustrum, a polygon which is not can almost contain twice as much vertexes
   * after clipping to a frustrum
   * 
  ...
  /*
   * Calculate the normal vector of the polygon, the resulting vector will face
   * away from aCenterPoint. aCenterPoint could be the center of the sphere the
   * polygon is part of.
   */
  public void calculateNormalVector(Point3D aCenterPoint, boolean toWardsPoint) throws PolygonException {
    if (mySize >= 3) {
      if (aCenterPoint != null)
        myNormalVector = new GVector(w[0].myPoint, w[1].myPoint, w[2].myPoint, aCenterPoint, toWardsPoint);
      // Debug.log(this,"calculatig normal vector with 4 points");
      else
        myNormalVector = new GVector(w[0].myPoint, w[1].myPoint, w[2].myPoint);
   ...
   */
  /*
   * public void translate(Camera aCamera) throws PolygonException,
   * MatrixException{ if(!isOptimized) optimize(); int i = 0; while(i < mySize){
   * w[i] = aCamera.world2Cam(w[i]); i++; } if(myNormalVector == null)
   * calculateNormalVector(); //TODO shoud this not be if else? else
   * myNormalVector = aCamera.world2Cam(myNormalVector); myCenterPoint =
   * aCamera.world2Cam(myCenterPoint); }
   */
  ...
  public void clip2Plane(Plane aPlane) throws PolygonException {
    ...
    for (int i = 0; i < myCamSize; i++) {
      ...
      // System.out.println("Distance of point: " + c[i].myPoint.toString() + " to
      // plane: " + aPlane.toString() + ": " + dist1);
      dist2 = aPlane.distanceToPoint(c[ii].myPoint);
      // System.out.println("Distance of point: " + c[ii].myPoint.toString() + " to
      // plane: " + aPlane.toString() + ": " + dist2);
      // System.out.println("Dist1: " + dist1 + " dist2: " + dist2);
      if (dist1 < 0 && dist2 < 0) {
        // both ends are on the wrong side of the plane, do nothing, the points will be
        // discarded
      } else if (dist1 >= 0 && dist2 >= 0) {
        // both ends are on the right side of the plane, copy the first point to the
        // temp vertexes array
        theTempVertexes[j++] = c[i];
      } else if (dist1 > 0) {
        // if we get here we know for sure that both ends are on a different side of the
        // plane
        // since dist1 > 0 we now that the first point is on the right side and the
        // second is on the wrong side
        ...
        // insert a new vertex at the point where the line from the first vertex to the
        // second intersects the plane
        theTempVertexes[j] = new Vertex(
            new Point3D(c[i].myPoint.x + (c[ii].myPoint.x - c[i].myPoint.x) * distratio,
        ...
        // insert a new Vertex there where the line from the first vertex to the second
        // vertex intersects the plane
        // unlike the previous situation we do not add the point on the right side of
        // the plane, it will be added in the next
        // iteration
        ...
        theTempVertexes[j] = new Vertex(
            new Point3D(c[ii].myPoint.x + (c[i].myPoint.x - c[ii].myPoint.x) * distratio,
  ...
  public List<Polygon> triangulate() {
    ...
    if (w.length > 3) {
      ...
      if (theSideLength > TRIANGULATION_THRESHOLD) {
        // only triangulate when the longest side of the triangle is greater then the
