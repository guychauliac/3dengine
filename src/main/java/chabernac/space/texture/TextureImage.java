...
public class TextureImage {
	...
	public static int[] getRGB(BufferedImage anImage) {
		...
		if (theType == DataBuffer.TYPE_BYTE && theBands > 1) {
			...
			for (int i = 0, j = 0; i < theData.length; i += theBands, j++) {
				...
				// alpha = (byte)240;
				// theColors[j] = (alpha << 24 & 0xFF000000) | (red << 16 & 0x00FF0000) | (green
				// << 8 & 0x0000FF00) | (blue << 0 & 0x000000FF);
				theColors[j] = (red << 24 & 0xFF000000) | (green << 16 & 0x00FF0000) | (blue << 8 & 0x0000FF00)
						| (alpha << 0 & 0x000000FF);
				// theColors[j] = (red << 16 & 0x00FF0000) | (green << 8 & 0x0000FF00) | (blue
				// << 0 & 0x000000FF);
				// theColors[j] = 0x00FF0000;
			}
		} else {
			System.out.println("Loading image in slow way");
			for (int x = 0; x < theWidth; x++) {
				for (int y = 0; y < theHeight; y++) {
					theColors[y * theWidth + x] = convertToRGBA(anImage.getRGB(x, y));
				}
			}
		}
	...
	private static int convertToRGBA(int aColor) {
		return chabernac.utils.Color.fromARGB(aColor).getRGBA();
	}
